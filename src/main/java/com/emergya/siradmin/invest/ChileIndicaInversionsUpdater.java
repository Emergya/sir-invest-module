...
public class ChileIndicaInversionsUpdater {
    ...
    public enum InversionType {
        ARI("ari"), PROPIR("propir"), EJECUCION("ejecucion"), PREINVERSION("preinversion");
    ...
    private static final Log LOGGER = LogFactory.getLog(ChileIndicaInversionsUpdater.class);
    ...
    public void getExistingKeysInChileindica() {
        ...
        for (WSURLRegionsCts regionName : WSURLRegionsCts.values()) {
            ...
            for (InversionType inversionType : InversionType.values()) {
                ...
                wsUrlBuilder.append(WS_PATH).append(regionName.toString().toLowerCase()).append(".").append(inversionType.getValue());
                ...
                if (wsUrl != null && !"".equals(wsUrl)) {
                    ...
                    ResponseEntity<ChileIndicaInversionsDataDto[]> responseEntity = restTemplate.exchange(wsUrl, HttpMethod.GET, entity,
                            ChileIndicaInversionsDataDto[].class);
                    if (responseEntity != null && HttpStatus.OK.equals(responseEntity.getStatusCode())) {
                        ChileIndicaInversionsDataDto[] regionInversionTypeProjects = responseEntity.getBody();
                        if (regionInversionTypeProjects != null && regionInversionTypeProjects.length > 0) {
                            for (ChileIndicaInversionsDataDto inversionProject : regionInversionTypeProjects) {

                                if (inversionProject.getGeoreferenciacion() != null && (inversionProject.getGeoreferenciacion()).length > 0) {
                                    if (inversionProject.getGeoreferenciacion()[0].getOgc() != null
                                            && inversionProject.getGeoreferenciacion()[0].getX() != null
                                            && !"".equals(inversionProject.getGeoreferenciacion()[0].getX())
                                            && inversionProject.getGeoreferenciacion()[0].getY() != null
                                            && !"".equals(inversionProject.getGeoreferenciacion()[0].getY())) {
                                        Geometry geometry = buildGeometry(inversionProject.getGeoreferenciacion()[0].getX(),
                                                inversionProject.getGeoreferenciacion()[0].getY(), inversionProject.getGeoreferenciacion()[0].getOgc());
                                        inversionProject.setInversionGeoreferenciacion(geometry);
                                    }
                                }

                                if (inversionProject.getFechaRegistro() != null) {
                                    inversionProject.setFechaRegistroIniciativa(service.getValidDate(inversionProject.getFechaRegistro()));
                                }
        ...
        if (LOGGER.isInfoEnabled()) {
            ...
            LOGGER.info("******************************************************");
            LOGGER.info("Actualización finalizada a a las " + Calendar.getInstance().getTime());
        }
        // return result;
    }
    ...
    private List<HttpMessageConverter<?>> getMessageConverters() {
        ...
        converter.setSupportedMediaTypes(ImmutableList.of(new MediaType("application", "json", MappingJackson2HttpMessageConverter.DEFAULT_CHARSET),
    ...
    public void getWsDataAndUpdateDB(List<ChileIndicaInversionsDataDto> wsProjectList) {
        ...
        for (Iterator<Long> iterator = projectDbIds.iterator(); iterator.hasNext();) {
            ...
            ChileIndicaInversionsDataDto dto = (ChileIndicaInversionsDataDto) service.getById(dbId);
            if (LOGGER.isInfoEnabled()) {
                LOGGER.info("Tratando proyecto existente BBDD " + dto.getIdIniciativaInversion());
            }
            // Buscamos si el proyecto existe entre las claves actuales
            int foundKeyIndex = Collections.binarySearch(wsProjectList, dto);
            if (foundKeyIndex >= 0) {
                ChileIndicaInversionsDataDto foundKey = wsProjectList.get(foundKeyIndex);
                iterator.remove();
                ...
                boolean updatable = service.checkIfProjectMustBeUpdated(foundKey.getIdIniciativaInversion(), foundKey.getFechaRegistro());
                if (updatable) {
                    service.delete(dto);
                ...
                service.delete(dto);
                if (LOGGER.isInfoEnabled()) {
                    LOGGER.info("Se elimina el proyecto por no existir en la consulta de llaves " + dto);
                }
            }
        }
    ...
    private Geometry buildGeometry(final String x, final String y, final BigInteger ogc) {
    ...
    private Geometry buildPoint(final int srs, final String x, final String y) throws NoSuchAuthorityCodeException, FactoryException,
